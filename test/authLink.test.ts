...
describe('auth link with auth center', () => {
    ...
    afterEach(() => {
        jest.mock('../packages/apollo-links/src/auth-link/clusterAuthLink', () => jest.requireActual('../packages/apollo-links/src/auth-link/clusterAuthLink'));
        jest.resetModules();
        jest.resetAllMocks()
        jest.clearAllMocks()
    });
    ...
    it('use fallback url when no agreement available', async () => {
        ...
        const {dictHttpLink} = await import('../packages/apollo-links/src');
        const link = dictHttpLink({...options, logger: mockLogger, chainId: "0x1234", fallbackServiceUrl});
    ...
    it('should use fallback when failed to get token', async () => {
        jest.mock('../packages/apollo-links/src/auth-link/clusterAuthLink', () => {
            const originalModule = jest.requireActual('../packages/apollo-links/src/auth-link/clusterAuthLink');
            return {
