...
describe.only('auth link with auth center', () => {
  ...
  beforeAll(async () => {
    ...
    const backupDictionary = "https://api.subquery.network/sq/subquery/polkadot-dictionary";
    const httpOptions = { fetch, fetchOptions: { timeout: 3000 } };
    const options = { authUrl, projectChainId, httpOptions, deploymentId, backupDictionary, logger }
